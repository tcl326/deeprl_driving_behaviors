...
class TrafficEnvMultiAgentsSimplePreset(TrafficEnvMultiAgents):
    def __init__(self, mode="gui"):
        ...
        ego_vehicles_w = [EgoVehicle('ego_car_w', 'route_ws', 'EgoCarW', 245., 241., 0.)]

        ego_vehicles_n = [EgoVehicle('ego_car_n', 'route_ns', 'EgoCarN', 245., 241., 0.)]

        ego_vehicles_e = [EgoVehicle('ego_car_e', 'route_es', 'EgoCarE', 245., 241., 0.)]

        ego_vehicles_dict_master = {'s': ego_vehicles_s, 'w':ego_vehicles_w, 'n':ego_vehicles_n, 'e':ego_vehicles_e}

        super(TrafficEnvMultiAgentsSimplePreset, self).__init__(ego_vehicles_dict_master=ego_vehicles_dict_master, mode=mode, lights=lights, netfile=netfile,
