...
env = gym.make('Traffic-Multi-preset-gui-v0')
env.seed(args.seed)
...
num_agents = 4
actor_models_dict = {n:ActorNetwork() for n in range(num_agents)}
# actor_model = ActorNetwork()
# print(actor_models)
actor_optimizer_dict = {n:optim.Adam(actor_models_dict[n].parameters(), lr=0.001) for n in range(num_agents)}
...
def get_prob(state, agent_id):
    ...
    probs = actor_models_dict[agent_id](Variable(state).unsqueeze(0))
    return probs
...
def replay_memory(batch_size):
    ...
    for states_whole, actions_whole, rewards_whole, next_states_whole, done_list_whole in zip(states_whole_batch, actions_whole_batch, rewards_whole_batch, next_states_whole_batch, done_list_whole_batch):
        ...
        probs = {agent_id:None for agent_id in range(num_agents)}
        rewards_dict = {num:[] for num in range(num_agents)}
        critic_model.time_steps = len(actions_whole)
        for agent_id in range(num_agents):
            actor_models_dict[agent_id].time_steps = len(actions_whole)
            states = torch.from_numpy(states_whole[:, 2*agent_id:2*(agent_id+1),:,:]).float()
            probs[agent_id] = actor_models_dict[agent_id](Variable(states), agent_id)
    ...
    for agent_id in range(num_agents):
        actor_optimizer_dict[agent_id].zero_grad()
    critic_optimizer.zero_grad()
    # print(value_losses)
    # print(policy_losses)
    # for agent_id in range(num_agents):
    actor_loss = torch.stack(policy_losses).sum()/float(num_agents)/float(batch_size)
    actor_loss.backward()
#
    critic_loss = torch.stack(value_losses).sum()/float(num_agents)/float(batch_size)
    critic_loss.backward()

    for agent_id in range(num_agents):
        actor_optimizer_dict[agent_id].step()
    critic_optimizer.step()

    for agent_id in range(num_agents):
        actor_models_dict[agent_id].rewards = []
        actor_models_dict[agent_id].saved_actions = []
...
def main():
    ...
    now = datetime.now()
    log_path = "log/multiple_central_critic_buffer_no_random_multinetwork"
    ...
    for i_episode in count(1):
        live_agents = [n for n in range(num_agents)]
        states = env.reset()
        states = [resize(state,(84, 84)) for state in states]
        for n in range(num_agents):
            actor_models_dict[n].saved_actions = []
            actor_models_dict[n].rewards = []
            actor_models_dict[n].time_steps = 1
            actor_models_dict[n].reset = True
        ...
        for t in range(10000):  # Don't infinite loop while learning
            if t % BPTT == 0 and t != 0:
                ...
                for agent_id in live_agents:
                    (actor_models_dict[agent_id].h_n, actor_models_dict[agent_id].c_n) = repackage_hidden((actor_models_dict[agent_id].h_n, actor_models_dict[agent_id].c_n))

            actions = np.zeros(num_agents)
            probs = []
            actions_replay = [Variable(torch.from_numpy(np.array([0])).long()) for agent in range(num_agents)]
            ...
            for i, agent_id in enumerate(live_agents):
            ...
            for agent_id in live_agents:
                ...
                actor_models_dict[agent_id].rewards.append(rewards[agent_id])
            if done:
                break
        ...
        total_reward = np.sum([np.sum(actor_models_dict[idx].rewards) for idx in range(num_agents)])
        ...
        if i_episode % args.log_interval == 0:
            for n in range(num_agents):
                save_checkpoint({
                            'episode': i_episode,
                            'state_dict': actor_models_dict[n].state_dict(),
                            'best_reward': max_reward,
                            'optimizer': actor_optimizer_dict[n].state_dict(),
                        }, is_best, filename='policy_' + str(i_episode) + '_' + str(n) + '.pth.tar')
            save_checkpoint({
                        'episode': i_episode,
